...
const sbgnDir = baseDir + '/sbgn/';
const sbgnJSONDir = baseDir + '/sbgnjson/';

const files = fs.readdirSync(sbgnDir);

const sbgnToJSON = (files) => {
  for (let file of files) {
    const converted = convert(fs.readFileSync(sbgnDir + file, 'utf-8'));
    fs.writeFile(sbgnJSONDir + file + '.json', JSON.stringify(converted, null, 2), (err) => {
      if (err) {
        throw err;
      }
    });
